...
def process_session(iemocap_dir, labels_df, sr, sess):
    ...
    # Este es el nuevo para evitar ocultos
    orig_wav_files = [f for f in os.listdir(wav_file_path) if f[0] not in ('.')]
    for orig_wav_file in tqdm(orig_wav_files):
        try:
            orig_wav_vector, _sr = librosa.load(wav_file_path + orig_wav_file, sr=sr)
            orig_wav_file, file_format = orig_wav_file.split('.')
            for _, row in labels_df[labels_df['wav_file'].str.contains(orig_wav_file)].iterrows():
    ...
    with open('../data/pre-processed/audio_vectors_{}.pkl'.format(sess), 'wb') as f:
        ...
        print("Saved audio_vector_{}".format(sess))
